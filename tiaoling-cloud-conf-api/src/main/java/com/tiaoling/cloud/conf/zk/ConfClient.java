...
import com.tiaoling.cloud.core.utils.PropertiesUtil;
...
public class ConfClient {
    ...
    public static void set(String key, String value) {
        if (cache != null) {
            logger.info(">>>>>>>>>> tl-conf: 初始化配置: [{}:{}]", new Object[]{key, value});
    ...
    public static void update(String key, String value) {
        if (cache != null) {
            if (cache.get(key)!=null) {
                logger.info(">>>>>>>>>> tl-conf: 更新配置: [{}:{}]", new Object[]{key, value});
                cache.put(new Element(key, value));
            }
        }
    }
    ...
    public static boolean remove(String key) {
        if (cache != null) {
            logger.info(">>>>>>>>>> tl-conf: 删除配置：key ", key);
